...
function ChaptersContent() {
  ...
  const save = async (asDraft = false) => {
    ...
    try {
      const res = await fetch('/api/admin/chapters', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
      ...
      if (selectedBookId) { const cRes = await fetch(`/api/admin/chapters?bookId=${selectedBookId}`); setChapters(await cRes.json()); }
    } catch (e: any) { showToast('Save failed'); }
    setSaving(false);
  };
  const toggleStatus = async (ch: any) => {
    const newStatus = ch.status === 'published' ? 'draft' : 'published';
    await fetch('/api/admin/chapters', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: ch.id, status: newStatus, bookId: ch.bookId }) });
    load();
  };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.media?.path) { setEditing((prev: any) => ({ ...prev, chapterImage: data.media.path })); showToast('Image uploaded!'); }
      ...
      <div className="fade-in">
        ...
        <div className="card space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div><label className="form-label">Chapter Order</label><input type="number" className="form-input" value={editing?.chapterOrder || chapters.length + 1} onChange={e => setEditing({ ...editing, chapterOrder: e.target.value })} /><p className="form-hint">Lower numbers appear first.</p></div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div><label className="form-label">Book</label><select className="form-input" value={editing?.bookId || selectedBookId} onChange={e => setEditing({ ...editing, bookId: e.target.value })}><option value="">Select book</option>{books.map(b => <option key={b.id} value={b.id}>{b.title}</option>)}</select></div>
          ...
          <div><label className="form-label">Chapter Image (optional)</label><div className="flex items-start gap-4">{editing?.chapterImage && <img src={editing.chapterImage} alt="" className="w-40 h-24 object-cover rounded-lg border" />}<div className="flex-1"><input type="file" accept="image/*" onChange={handleImageUpload} className="form-input" /><input className="form-input mt-2" value={editing?.chapterImage || ''} onChange={e => setEditing({ ...editing, chapterImage: e.target.value })} placeholder="Or paste image URL" /></div></div></div>
          <div><label className="form-label">Content</label><p className="form-hint mb-2">Write your chapter content here.</p><RichEditor value={editing?.content || ''} onChange={v => setEditing({ ...editing, content: v })} minHeight="500px" /></div>
          <div className="flex gap-3 pt-2">
            <button onClick={() => save()} disabled={saving} className="btn btn-primary">{saving ? <><i className="fa-solid fa-spinner fa-spin mr-2"></i>Saving...</> : <><i className="fa-solid fa-check mr-2"></i>Save Chapter</>}</button>
  ...
  return (
    <div className="fade-in">
      ...
      <div className="card mb-4"><label className="form-label"><i className="fa-solid fa-book mr-2"></i>Select Book</label><select className="form-input max-w-md" value={selectedBookId} onChange={e => setSelectedBookId(e.target.value)}><option value="">Choose a book to manage chapters...</option>{books.map(b => <option key={b.id} value={b.id}>{b.title}</option>)}</select></div>
      {selectedBookId && !loading && (
        chapters.length === 0 ? (
          <div className="card text-center py-12"><i className="fa-solid fa-file-pen text-4xl mb-3" style={{ color: 'var(--text-muted)' }}></i><p style={{ color: 'var(--text-muted)' }}>No chapters for &quot;{bookName}&quot;. Add your first chapter!</p></div>
        ) : (
              ...
              <tbody>
                {chapters.map(c => (
                  <tr key={c.id}>
                    ...
                    <td><button onClick={() => toggleStatus(c)} className={`badge ${c.status === 'published' ? 'badge-green' : 'badge-yellow'}`} style={{ cursor: 'pointer', border: 'none' }} title="Click to toggle">{c.status}</button></td>
                    <td><div className="flex gap-2"><button onClick={() => { setEditing(c); setShowForm(true); }} className="btn btn-sm btn-secondary"><i className="fa-solid fa-pen"></i></button><button onClick={() => remove(c.id)} className="btn btn-sm btn-danger"><i className="fa-solid fa-trash"></i></button></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export default function AdminChapters() {
  return <Suspense fallback={<div className="page-loader"><i className="fa-solid fa-spinner fa-spin text-2xl"></i></div>}><ChaptersContent /></Suspense>;
}
