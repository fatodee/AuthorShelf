...
export default function AdminBooks() {
  ...
  const save = async () => {
    ...
    await fetch('/api/admin/books', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
    setSaving(false);
    setShowForm(false);
    setEditing(null);
    load();
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (data.media?.path) setEditing({ ...editing, coverImage: data.media.path });
  };
        ...
        <div className="card space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div>
              ...
              <input className="form-input" value={editing?.title || ''} onChange={e => setEditing({ ...editing, title: e.target.value, slug: editing?.id ? editing.slug : generateSlug(e.target.value) })} />
            </div>
            <div>
            ...
            <div>
              ...
              <select className="form-input" value={editing?.categoryId || ''} onChange={e => setEditing({ ...editing, categoryId: e.target.value })}>
            ...
            <div>
              ...
              <select className="form-input" value={editing?.bookType || 'series'} onChange={e => setEditing({ ...editing, bookType: e.target.value })}>
                <option value="series">Series (multiple chapters)</option>
                <option value="single">Single Story (one piece)</option>
              </select>
              <p className="form-hint">Series = add chapters later. Single = the description IS the story.</p>
            </div>
            <div>
            ...
            <div className="flex items-center gap-3 pt-7">
              <button type="button" className={`toggle ${editing?.featured ? 'active' : ''}`} onClick={() => setEditing({ ...editing, featured: !editing?.featured })} />
              <label className="text-sm font-medium">Featured Book</label>
              <span className="form-hint">(Shows in featured section on homepage)</span>
            </div>
          </div>
          <div>
            <label className="form-label">Cover Image</label>
            <div className="flex items-start gap-4">
              {editing?.coverImage && <img src={editing.coverImage} alt="Cover" className="w-32 h-44 object-cover rounded-lg border" />}
              <div>
                <input type="file" accept="image/*" onChange={handleImageUpload} className="form-input" />
                <p className="form-hint">Or paste a URL:</p>
                <input className="form-input mt-1" value={editing?.coverImage || ''} onChange={e => setEditing({ ...editing, coverImage: e.target.value })} placeholder="https://..." />
              </div>
            </div>
          </div>
          <div>
            <label className="form-label">Description</label>
            <RichEditor value={editing?.description || ''} onChange={v => setEditing({ ...editing, description: v })} />
          </div>
          <div>
            <label className="form-label">Author Note (optional)</label>
            <RichEditor value={editing?.authorNote || ''} onChange={v => setEditing({ ...editing, authorNote: v })} minHeight="120px" />
            <p className="form-hint">A personal note about this book, shown on the book detail page.</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div>
              <label className="form-label">SEO Title</label>
              <input className="form-input" value={editing?.metaTitle || ''} onChange={e => setEditing({ ...editing, metaTitle: e.target.value })} />
            </div>
            <div>
              <label className="form-label">SEO Description</label>
              <input className="form-input" value={editing?.metaDescription || ''} onChange={e => setEditing({ ...editing, metaDescription: e.target.value })} />
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <button onClick={save} disabled={saving} className="btn btn-primary">
              {saving ? <><i className="fa-solid fa-spinner fa-spin mr-2"></i>Saving...</> : <><i className="fa-solid fa-check mr-2"></i>Save Book</>}
            </button>
    ...
    <div className="fade-in">
      ...
      {books.length === 0 ? (
        <div className="card text-center py-12">
          <i className="fa-solid fa-book-open text-4xl mb-3" style={{ color: 'var(--text-muted)' }}></i>
          <p style={{ color: 'var(--text-muted)' }}>No books yet. Create your first book!</p>
        </div>
      ) : (
